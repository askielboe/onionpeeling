proc peelingfake { args } {
	...
	for {set alpha $alphamin} {$alpha <= $alphamax} {set alpha [expr $alpha + $alphastep]} {
		...
		set fit_temp_error($i,$ishell) [geterror 1]
		set fit_norm($i,$ishell) [getpar 6]
		set fit_norm_error($i,$ishell) [geterror 6]
	...
	for {set n 1} {$n < $nshells} {incr n} {
		...
		puts $fout "$radius($n) $fit_temp($imin,$n) $fit_temp_error($imin,$n)"
	}
	close $fout
	...
	for {set n 1} {$n < $nshells} {incr n} {
		...
		puts $fout "$radius($n) $fit_norm($imin,$n) $fit_norm_error($imin,$n)"
	}
	close $fout
